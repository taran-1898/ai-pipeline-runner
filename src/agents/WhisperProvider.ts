...
export class WhisperProvider implements SpeechToTextProvider {
  ...
  async transcribeBuffer(audioBuffer: Buffer, mimeType: string): Promise<string> {
    if (!config.whisperApiKey) {
      return `[Whisper stubbed transcript for audio buffer of size ${audioBuffer.length}]`;
    }
    ...
    // The OpenAI transcription API expects a filename
    form.append("file", blob, `audio.${extension}`);
    // Using standard whisper-1 here
